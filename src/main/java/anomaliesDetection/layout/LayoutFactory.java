...
public class LayoutFactory {
    ...
    public void buildRTree(String dom) {
        ...
        try {
            ...
            System.err.println("JSON Exception while anomaliesDetection.layout : \n" + dom);
            ex2.printStackTrace();
        }
    }
                            ...
                            if (!ArrayUtils.contains(tagsIgnore, parseTagName(xpath).toUpperCase())) {
                                ...
                                if (!childOfNoSizeElement(xpath)) {
                                    try {
                                        ...
                                        System.out.println("Error while anomaliesDetection.layout the XPath of " + obj.toString());
                                        e.printStackTrace();
                                    }
                                }
                            }
    ...
    public static int[] getCoords(JSONObject ob, boolean b) {
        try {
            ...
            System.out.println("Error while anomaliesDetection.layout coordinates");
            e.printStackTrace();
        }
        return null;
    }
